...
\item We conduct experiments on 2025 Dow-30 equities against an identical-protocol suite of real trained time-series models and multi-trade agent baselines, with ablations and a multi-window robustness study, showing that \textsc{FinOPD} attains the highest portfolio Sharpe ratio, cumulative return, and Calmar ratio of any method, and that a hindsight-conditioned teacher characterizes the achievable performance ceiling toward which self-evolution drives the policy.
\end{itemize}
% 中文：我们的贡献如下：(1) 形式化金融多智能体系统的自进化问题，识别三个设计问题（特权信息、信用分配、参数充分性）使其区别于单模型自蒸馏；(2) 提出 FinOPD，将在线策略自蒸馏与事后 PnL 条件化、Shapley 加权信用分配和检索增强 belief 存储整合为统一的双轨进化框架；(3) 在 2025 年 Dow-30 股票上、对由真实训练时序模型与多笔交易 agent 基线构成的同协议套件进行实验，含消融与多窗口稳健性研究，表明 FinOPD 取得最高组合夏普和跨评测窗口最稳定的风险调整表现，同时相对被动基准显著降低回撤。
