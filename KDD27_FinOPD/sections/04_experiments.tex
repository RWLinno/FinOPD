...
\paragraph{Portfolio-Level Performance.} Table~\ref{tab:overall} reports results on a representative Dow-30 basket (GOOGL/JNJ/GS/DIS, equal-weight). \textsc{FinOPD} achieves the highest Sharpe ratio (1.76) and best Calmar ratio (4.7) among all methods while maintaining the lowest maximum drawdown (9.4\%), evaluated with the full Qwen2.5-VL-7B + OPD-LoRA multi-agent pipeline over 142 evolved factors. The advantage is risk-adjusted: \textsc{FinOPD} pairs competitive cumulative return (38.3\%) with disciplined downside control, whereas time-series models such as TimesNet reach comparable raw return but at substantially higher drawdown (13.1\%). Among LLM multi-agent baselines, performance is uneven across assets, reflecting their reliance on textual signals that do not transfer uniformly across sectors.
% 中文：表1报告代表性 Dow-30 组合(GOOGL/JNJ/GS/DIS 等权)。FinOPD 用完整 Qwen2.5-VL-7B + OPD-LoRA 多智能体流水线(142 进化因子)取得最高夏普(1.76)和最佳 Calmar(4.7),同时保持最低回撤(9.4%)。优势在于风险调整:FinOPD 在有竞争力的累计收益(38.3%)下严格控制下行,而 TimesNet 等时序模型虽收益相近但回撤显著更高(13.1%)。LLM 多智能体基线跨资产表现不均,反映其对文本信号的依赖难以跨板块一致迁移。
...
\paragraph{Per-Asset Heterogeneity.} Table~\ref{tab:per_asset} provides per-asset breakdowns for representative equities. \textsc{FinOPD} attains strong risk-adjusted returns on trending large-caps (GOOGL SR 2.47 with 77.5\% cumulative return; JNJ SR 2.01) and remains the only method to stay profitable with the lowest drawdown on the more difficult DIS, where several baselines incur losses. The visual-geometric perception and 142 evolved factors jointly drive directional accuracy, while the edge-gated position policy converts confirmed trends into full-position exposure. On assets where simple momentum already captures most of the move (e.g., GS), specialized time-series models can reach comparable Sharpe, but \textsc{FinOPD} maintains a more favorable drawdown profile.
% 中文：表2提供代表性个股的逐资产分解。FinOPD 在趋势型大盘股上取得强风险调整收益(GOOGL 夏普 2.47、累计收益 77.5%;JNJ 夏普 2.01),并在更困难的 DIS 上以最低回撤保持唯一盈利(多个基线在此亏损)。视觉几何感知与 142 个进化因子共同驱动方向准确率,边际门控仓位策略把确认的趋势转化为满仓暴露。在简单动量已能捕获大部分行情的资产(如 GS)上,专用时序模型可达相近夏普,但 FinOPD 的回撤更优。
