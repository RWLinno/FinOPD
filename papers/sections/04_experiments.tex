...
\paragraph{Aligned training data for visual SFT.}
For visual SFT, we construct point-in-time aligned triples of financial time series, filing text, and rendered charts. Each chart is accompanied by local bounding boxes mapped to quantitative factors. The training protocol freezes the text and time-series encoders and updates only the visual encoder. This protocol allows direct attribution between local chart evidence and factor-level supervision while keeping non-visual representations stable.
