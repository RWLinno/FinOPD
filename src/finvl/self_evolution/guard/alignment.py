...
class AlignmentGuard:
    ...
    def _estimate_kl(self) -> float:
        """Estimate current KL divergence (placeholder)."""
        if not self._kl_history:
            return 0.01
        base = self._kl_history[-1] if self._kl_history else 0.01
        return base + np.random.exponential(0.005)
    ...
    def _adversarial_refresh(self, iteration: int):
        ...
        """
        logger.info(
            f"Adversarial refresh at iteration {iteration} "
            f"using dates: {self.adversarial_dates}"
        )
